...
class FPNSSD512(nn.Module):
    ...
    def __init__(self, num_classes, fpn_net, head_channels=256):
    ...
    def _make_head(self, out_planes):
        layers = []
        for _ in range(4):
            layers.append(nn.Conv2d(self.head_channels, self.head_channels, kernel_size=3, stride=1, padding=1))
            layers.append(nn.ReLU(True))
        layers.append(nn.Conv2d(self.head_channels, out_planes, kernel_size=3, stride=1, padding=1))
